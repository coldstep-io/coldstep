...
func TestPreferRunError_Precedence(t *testing.T) {
	...
	denyTCP := newDefendDenyError(telemetry.DenyEvent{Protocol: "tcp", Dst: "1.2.3.4", Dport: 443})
	denyUDP := newDefendDenyError(telemetry.DenyEvent{Protocol: "udp", Dst: "8.8.8.8", Dport: 53})
	...
	tests := []struct {
		...
		{"defend deny replaces plain", plain, denyTCP, denyTCP},
		{"defend deny wins over context.Canceled as current", context.Canceled, denyTCP, denyTCP},
		{"keep plain when current is deny", denyTCP, plain, denyTCP},
		{"defend deny vs defend deny keeps current", denyTCP, denyUDP, denyTCP},
	}
