...
Cgroup programs (`trace_defend.bpf.c`) and LSM (`trace_lsm_defend.bpf.c`) apply IPv4 allowlist semantics separately from syscall telemetry; shared helpers live in `bpf/defend_policy.inc`. Parity gaps (for example hook coverage vs detect paths) should be tracked as explicit decisions, not assumed.
