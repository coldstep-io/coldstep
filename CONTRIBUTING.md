...
2. **Go** — CI uses **`setup-go`** with **`go-version-file: go.mod`** (matches the **`go`** directive, currently **1.25.11**); see **`.github/workflows/coldstep-ci-runner.yml`**. After Linux prep, `gofmt`, `go vet ./...`, and `go test ./...` should pass (see CI for integration tags).
...
6. **Pinning for consumers** — downstream workflows should use a **release tag** (for example **`coldstep-io/coldstep@v0.6.0`**), not **`@main`**, unless they intentionally track head.
