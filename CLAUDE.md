...
`action.yml` inputs: `mode`, `allow`, `allow-file`, `detect-profile`, `report`, `fail-on-error`, `signing-key`, `log-level`, `no-default-ignored-nets`, `ready-timeout-seconds`, `github-token`. Internal-only: `phase`, `release-path`, `smoke-test-egress`, `io-uring-disable`. The allowlist is one model: `allow` / `allow-file` carry domains, wildcards, IPv4/CIDRs, and `!CIDR` ignore entries (the only ignore mechanism); `no-default-ignored-nets` toggles the implicit RFC1918 ignores. Removed inputs (consumers warned at start if still set): `ignored-nets`, `ignored-nets-file`, `bootstrap-allowlist` (fold into `allow`/`allow-file`); and the earlier deprecated aliases (`allowed-domains*`, `allowed-hosts*`, `allowed-ips*`, `ignored-ip-nets*`, `report-job-summary`, `report-pr-summary`, `feature-gates`).
...
Nine BPF probe packages under `internal/bpf/`, each compiled by `go generate` (plus the `bpfgen` code-gen helper, which is not itself a probe):

| Package | Compiler driver | Why |
| :------ | :------------- | :-- |
| `traceexec`, `tracefork`, `traceipv6` | direct `//go:generate go run github.com/cilium/ebpf/cmd/bpf2go@v0.21.0 …` | No syscall-NR dispatch in the C source, so no per-arch flags needed. |
| `traceconnect`, `tracedns`, `tracefs`, `tracebpfaudit`, `tracektls`, `defend` | indirect `//go:generate go run ../bpfgen/main.go` | C sources branch on syscall numbers per arch (`#if defined(bpf_target_arm64)` vs `bpf_target_x86`) so bpf2go must be invoked with `-D__TARGET_ARCH_<runtime.GOARCH>`; the wrapper builds the flags string. `defend` only needs this because its LSM section pulls in `trace_connect_obs.h`. |

Shared C headers live in `bpf/` (notably `coldstep_pure.h`, `deny_event.h`, `defend_policy.inc`, `defend_lpm_key.h`, `trace_connect_obs.h`, `trace_*_obs.inc`, `trace_tls_write.inc`). Defend hooks come from one combined source `trace_defend_all.bpf.c`, which includes `trace_defend_cgroup.inc` (cgroup `connect4`/`sendmsg4` plus `connect6`/`sendmsg6`) and `trace_lsm_defend_lsm.inc` (BPF LSM `socket_connect`/`socket_sendmsg`, IPv4 **and** IPv6), plus `trace_lsm_bpf_self_defense.inc` (BPF LSM `bpf` — self-defense). The self-defense hook denies `BPF_PROG_GET_FD_BY_ID` / `BPF_MAP_GET_FD_BY_ID` / `BPF_OBJ_GET` that target coldstep's **own** object ids (recorded into `self_prog_ids`/`self_map_ids` by `armBpfSelfDefense` at startup) by a non-agent task, blocking a CAP_BPF attacker from grabbing a handle to detach the monitor; it is armed (`self_defense_cfg.enabled=1`) only after the id sets are populated, the agent's own tgid is exempt, and it emits a `bpf_self_defense` JSONL row + digest KPI on each denial. Like every LSM hook here it only **enforces** where `bpf` is in the kernel boot `lsm=` chain (`/sys/kernel/security/lsm`) — GH hosted runners boot without it, so the hook attaches but stays silent there; the deny integration test gates on that and skips otherwise. The cgroup section enforces **IPv4, IPv4-mapped IPv6, and native IPv6**: the `connect6`/`sendmsg6` hooks (`defend_cgroup_sock_addr_ipv6`, `bpf/trace_defend_cgroup.inc:296-339`) gate v4-mapped destinations against the IPv4 allowlist and native IPv6 against the `allowed_ipv6` LPM trie, with `::1` (loopback) and `fe80::/10` (link-local) always bypassing. **IPv4 loopback (`127.0.0.0/8`) is always bypassed** via `coldstep_ipv4_is_loopback` (`bpf/coldstep_pure.h`, host-tested) checked at the top of `dst_in_ignored` in `bpf/defend_policy.inc` (shared by cgroup, LSM, and SKB paths) — `127.0.0.53` is the systemd-resolved DNS stub and blocking it severs all external DNS for every process in the cgroup. **The LSM section twins this for native IPv6**: `lsm_socket_connect`/`lsm_socket_sendmsg` (`bpf/trace_lsm_defend_lsm.inc`) read the 16-byte v6 dst (sockaddr_in6 or `skc_v6_daddr` fallback) and call `lsm_v6_enforce`, which gates v4-mapped against the IPv4 allowlist and native IPv6 against `allowed_ipv6` (same `::1`/`fe80::/10` bypass), emitting an AF_INET6 deny into the LSM's own `lsm_deny_events` ring. Like all LSM hooks it only **enforces** where `bpf` is in the lsm boot chain. The Go-side loader `defend.LoadDefendObjectsForKernel` strips the LSM section from the spec on kernels without CONFIG_BPF_LSM so prog_load doesn't fail.
...
- **Go version:** `go 1.25.13` pinned in `go.mod`; CI uses `setup-go` with `go-version-file: go.mod`. Don't bump the minor in code without updating workflows.
...
- **Docs alignment:** changes to `action.yml` inputs or workflow pins require updating `README.md`, `QUICK_START.md`, and the input descriptions in lockstep. When a change alters defend-mode **enforcement scope** (new hook, protocol family, bypass), update the coverage claims in `action.yml`'s `mode` description, `README.md`, and `SECURITY.md` in the same PR — stale "X is not blocked" claims cause doc drift. Release pins (`MARKETPLACE_COLDSTEP_TAG`, `COLDSTEP_AGENT_VERSION`, `website/`) follow a two-train flow — repo docs + CI pins land in the release PR before `git tag`; `website/` bumps land in a **separate follow-up PR after** the tag exists on Releases.
- **Release version invariant (NEVER violate):** the commit a `vX.Y.Z` tag points to must carry `COLDSTEP_BINARY_VERSION = 'vX.Y.Z'` in `src/shared.ts` **and** in the compiled `dist/` bundles, and `MARKETPLACE_COLDSTEP_TAG` (`scripts/check_workflow_action_pins.py`) must equal it. The bump + `npm run build` dist rebuild go in the **release PR, before the tag** — there is no post-tag binary bump. Enforced three ways; never weaken or remove them: `scripts/check_release_version_alignment.py` in the `action_bundle` CI job (every PR, alongside `check_workflow_action_pins.py --skip-website`), the same script with `--tag` as the **first step** of `supply-chain-attest.yml` (fails a misaligned release before any asset is built/uploaded), and the `internal/releasecheck` Go test in `go test ./...`. Never re-point a published tag; cut the next patch version instead.
