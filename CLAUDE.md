...
- **Docs alignment:** changes to `action.yml` inputs or workflow pins require updating `README.md`, `QUICK_START.md`, and the input descriptions in lockstep. When a change alters defend-mode **enforcement scope** (new hook, protocol family, bypass), update the coverage claims in `action.yml`'s `mode` description, `README.md`, and `SECURITY.md` in the same PR — stale "X is not blocked" claims are how the v0.5.2 IPv6 doc drift shipped. Release pins (`MARKETPLACE_COLDSTEP_TAG`, `COLDSTEP_AGENT_VERSION`, `website/`) follow the two-train flow in `RELEASE_PROCESS.md` — repo docs + CI pins land in the release PR before `git tag`; `website/` bumps land in a **separate follow-up PR after** the tag exists on Releases.
- **Release version invariant (NEVER violate; v0.5.2 incident):** the commit a `vX.Y.Z` tag points to must carry `COLDSTEP_BINARY_VERSION = 'vX.Y.Z'` in `src/shared.ts` **and** in the compiled `dist/` bundles, and `MARKETPLACE_COLDSTEP_TAG` (`scripts/check_workflow_action_pins.py`) must equal it. The bump + `npm run build` dist rebuild go in the **release PR, before the tag** — there is no post-tag "Train-2" binary bump (that flow shipped the v0.5.1 agent under the v0.5.2 tag). Enforced three ways; never weaken or remove them: `scripts/check_release_version_alignment.py` in the `action_bundle` CI job (every PR, alongside `check_workflow_action_pins.py --skip-website`), the same script with `--tag` as the **first step** of `supply-chain-attest.yml` (fails a misaligned release before any asset is built/uploaded), and the `internal/releasecheck` Go test in `go test ./...`. Never re-point a published tag; cut the next patch version instead. Full incident write-up: `RELEASE_PROCESS.md` → "Reference: v0.5.2 incident".
